...
import am.immodule.Model.Evens.UnreadEvent;
...
public class IMUnreadMsgManager extends IMManager {
    ...
    public void onRepUnreadMsgContactList(IMMessage.IMUnreadMsgCntRsp unreadMsgCntRsp) {
        ...
        Log.d("zzzzz"," totalUnreadCount "+totalUnreadCount+" --- "+unreadInfoList.size());
        Log.d("zzzzz"," attachData "+new String(unreadMsgCntRsp.getAttachData().toByteArray()));

        for(IMBaseDefine.UnreadInfo u : unreadInfoList){
            Log.d("zzzzz","getLatestMsgData "+new String(u.getLatestMsgData().toByteArray()));
            Log.d("zzzzz","getLatestMsgFromUserId "+u.getLatestMsgFromUserId());
            Log.d("zzzzz","getLatestMsgId "+u.getLatestMsgId());
        }
        Log.d("zzzzz","unread#unreadInfoList "+unreadInfoList.toString());
        Log.d("zzzzz","unread#unreadMsgCnt:%d, unreadMsgInfoCnt:%d "+unreadInfoList.size());
        ...
        for(IMBaseDefine.UnreadInfo unreadInfo:unreadInfoList){
            ...
            Log.d("zzzzz","unreadEntity "+unreadEntity.toString());
            //屏蔽的设定
    ...
    public void  readUnreadSession(String sessionKey){
        logger.d("unread#readUnreadSession# sessionKey "+sessionKey);
         if(unreadMsgMap.containsKey(sessionKey)){
