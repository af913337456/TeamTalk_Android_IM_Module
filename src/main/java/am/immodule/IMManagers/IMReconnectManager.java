...
import am.immodule.Common.NetworkUtil;
import am.immodule.Model.Evens.LoginEvent;
import am.immodule.Model.Evens.ReconnectEvent;
...
public class IMReconnectManager extends IMManager {
    ...
    public void onEventMainThread(SocketEvent socketEvent){
        ...
        switch (socketEvent){
            case MSG_SERVER_DISCONNECTED: /** 心跳检查失败重连 */
    ...
    public void onAction(String action, Intent intent) {
        ...
        if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION)) { /** 网络更改的广播监听 */
            logger.d("reconnect#onAction#网络状态发生变化!!");
            tryReconnect();
        } else if (action.equals(ACTION_RECONNECT)) {       /** 重连 */
            isAlarmTrigger = true;
            tryReconnect();
        }
    }
