package am.immodule.IMManagers;
...
import am.immodule.Callback.ListenerQueue;
import am.immodule.Callback.Packetlistener;
import am.immodule.Common.Logger;
import am.immodule.Common.SysConstant;
import am.immodule.Common.SystemConfigSp;
import am.immodule.Common.UrlConstant;
import am.immodule.Core.DataBuffer;
import am.immodule.Core.DefaultHeader;
import am.immodule.Core.Header;
import am.immodule.Core.IMBaseDefine;
import am.immodule.Core.MsgServerHandler;
import am.immodule.Core.SocketThread;
import am.immodule.Model.Evens.SocketEvent;
...
public class IMSocketManager extends IMManager {
    ...
    public void sendRequest(GeneratedMessageLite requset,int sid,int cid){
        Log.d("zzzzz","sendRequest(requset,sid,cid,null)");
    ...
    public void packetDispatch(ChannelBuffer channelBuffer){
        ...
        Log.d("zzzzz","dispatch packet, serviceId:%d, commandId:%d"+serviceId+"---"+commandId);
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            try {
                logger.d("socket#req msgAddress onSuccess, response:%s", msg.obj);
...
//                msgServer.backupIP = jsonObject.getString("backupIP");
//                msgServer.code = jsonObject.getInt("code");
//                msgServer.priorIP = jsonObject.getString("priorIP");
//                msgServer.port = Integer.parseInt(jsonObject.getString("port"));
//                msgServer.msg = jsonObject.getString("msg");

                if(msgServer == null){
                    triggerEvent(SocketEvent.REQ_MSG_SERVER_ADDRS_FAILED);
                    return;
                }
                connectMsgServer(msgServer); /** socket 初始化入口 */
                triggerEvent(SocketEvent.REQ_MSG_SERVER_ADDRS_SUCCESS);
            }catch (Exception e){
                logger.d("handleMessage Exception "+e.toString());
            }
        }
    };

    /** 減去 async-http 的依賴，一次性請求 */
    private void helperGet(String url){
        try{
            URL myUrl = new URL(url);
            HttpURLConnection httpURLConnection = (HttpURLConnection) myUrl.openConnection();
            httpURLConnection.setRequestMethod("GET");
            httpURLConnection.setConnectTimeout(5*1000);
            httpURLConnection.setRequestProperty("User-Agent","Android-TT");
            ...
            message.obj = stringBuilder.toString().trim();
            handler.sendMessage(message);
        }catch (Exception e){
            logger.d("get server exception is "+e.toString());
        }
    }

    /** 全局的 socket 在这个函数里面进行初始化 */
    ...
    public void reqMsgServerAddrs() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                helperGet(SystemConfigSp.instance().getStrConfig(SystemConfigSp.SysCfgDimension.LOGINSERVER));
            }
        }).start();
//        client.setUserAgent("Android-TT");
//        Log.d("zzzzz","url is "+SystemConfigSp.instance().getStrConfig(SystemConfigSp.SysCfgDimension.LOGINSERVER));
//        client.get(
//                /** 第一个参数 URL 就是输入框里面的连接 */
...
//                        logger.d("socket#req msgAddress onSuccess, response:%s", s.toString());
