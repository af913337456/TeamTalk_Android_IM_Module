...
import am.immodule.Model.Beans.Util.EntityChangeEngine;
import am.immodule.Model.Evens.MessageEvent;
import am.immodule.Model.Evens.PriorityEvent;
import am.immodule.Model.Evens.RefreshHistoryMsgEvent;
import am.immodule.Server.LoadImageService;
...
public class IMMessageManager extends IMManager{
    ...
    public void ackReceiveMsg(MessageEntity msg){
        ...
        IMMessage
                .IMMsgDataAck imMsgDataAck = IMMessage.IMMsgDataAck.newBuilder()
    ...
    public void sendMessage(MessageEntity msgEntity) {
        ...
        imSocketManager.sendRequest(
                ...
                sid,
                cid,
                new Packetlistener(getTimeoutTolerance(messageEntity))
        {
            @Override
    ...
    public void sendImages(List<ImageMessage> msgList) {
		...
		for (ImageMessage msg : msgList) {
            ...
            switch (loadStatus){
                ...
                case MessageConstant.IMAGE_UNLOAD:         /** 还没上传状态 */
                case MessageConstant.IMAGE_LOADING:        /** 正在上传状态 */
    ...
    private List<MessageEntity> doLoadHistoryMsg(int pullTimes,final int peerId,final int peerType, final String sessionKey,int lastMsgId,int lastCreateTime,int count){
        ...
        // 降序结果输出desc
        logger.d("数据库读取的参数是：sessionKey lastMsgId lastCreateTime count "
                +"---"+sessionKey
                +"---"+lastMsgId
                +"---"+lastCreateTime
                +"---"+lastCreateTime
    ...
    public void onReqMsgById(IMMessage.IMGetMsgByIdRsp rsp){
        ...
        for(IMBaseDefine.MsgInfo msg:msgList){
            ...
            Log.d("zzzzz","-----内容-----"+entity.getContent());
            Log.d("zzzzz","-----显示-----"+entity.getMessageDisplay());
            Log.d("zzzzz","-----时间-----"+entity.getCreated());
            Log.d("zzzzz","-----类型-----"+entity.getMsgType());
            Log.d("zzzzz","-----会话-----"+sessionKey);
        ...
        Log.d("zzzzz","-----解析完毕-----");
        dbInterface.batchInsertOrUpdateMessage(dbEntity); /** 添加到本地数据库 */
