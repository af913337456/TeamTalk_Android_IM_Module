	...
	public boolean doConnect() {
		try {
					...
					&& null != this.strHost && this.nPort > 0)
            {
                ...
                //socketAddress = new InetSocketAddress(strHost, nPort);
				channelFuture = clientBootstrap.connect(socket.getRemoteSocketAddress());
...
//                bufferedOutputStream.write(buffer.getOrignalBuffer().array());
//                bufferedOutputStream.flush();
//                Log.d("zzzzz","packet#send ok new way ");
//            } catch (IOException e) {
//                Log.d("zzzzz","new way "+e.toString());
//                e.printStackTrace();
//            }
            channelFuture.getChannel().write(buffer.getOrignalBuffer()); /** 真正发送的地方 */
            Log.d("zzzzz","packet#send ok");
            return true;
        } else {
			Log.d("zzzzz","packet#send failed");
            return false;
        }
    }
